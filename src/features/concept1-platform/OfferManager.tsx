...
export default function OfferManager() {
  ...
  const handlePrint = useCallback((offer: Offer) => {
    ...
    printWindow.document.write(`
      <html>
        <head><title>QR Code - ${offer.code}</title></head>
        <body style="display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;">
          <div style="text-align:center;">
            ${svgData}
            <p style="margin-top:16px;font-family:sans-serif;font-size:14px;color:#666;">${offer.description}</p>
            <p style="font-family:monospace;font-size:18px;font-weight:bold;">${offer.code}</p>
          </div>
        </body>
      </html>
    `);
